...
/* Inizializza una struct sockaddr (allocata dal chiamante) con indirizzo e porta
 * Accetta sia indirizzi ip che nomi
 */
void myCreateSA_dns(struct sockaddr_in * sa, char * address , int port){
	...
	Getaddrinfo(address, buf, NULL, &list);
	solvedaddr = (struct sockaddr_in*)list->ai_addr;

	sa->sin_family = AF_INET;
	sa->sin_port = solvedaddr->sin_port;
	sa->sin_addr.s_addr = solvedaddr->sin_addr.s_addr;
 ...
 * 
 */
void myCreateSA(struct sockaddr_in * sa, char * address , int port){
       
      memset(sa,0,sizeof(struct sockaddr_in));
      sa->sin_family = AF_INET;
      sa->sin_port = htons(port);
      if (inet_aton(address,&(sa->sin_addr)) == 0){
        err_quit("(%s) Error inet_aton() in myCreateSA ",prog_name);
      }
      
     return;
}

void print_SA(const char *str,struct sockaddr_in *a){	
	printf("%s %s:%u\n",str,inet_ntoa(a->sin_addr), ntohs(a->sin_port));
	return;
}

void print_addr(char *str, struct sockaddr_in *a)
{
	/*DEPRECATA
	uint32_t ipaddr;
    unsigned char *p;
...
/*
 * Readline senza buffering dei dati
 */
//ritorna la riga letta. No buffer. No \n
...
//ritorna la riga letta. No buffer.
//timeout con select
ssize_t ReadlineNoBuf (int fd, void *ptr, size_t maxlen)
{
    ssize_t n;
...
void remove_r_n(char * str,int maxlen){
	int i ;	
	for (i=0;i<maxlen;i++){
		if (str[i] == '\0')
			break;
